...
public class CameraManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //top half screen
        viewPort[0, 0] = 0.0f; viewPort[0, 1] = 0.5f;
        //top right screen
        ...
        //bottom right screen
        viewPort[2, 0] = 0.0f; viewPort[2, 1] = -0.5f;
        //bottom right screen
        ...
        RespawnManager.Intance.playerSpawned += ResetCameraChange;
    } 
        ...
        while (changeCamera)
        {
            ...
            if (playerLength != playerCount && playerCount > 1)
            {
                ...
                for (int i = 0; i < playerCount; ++i)
                {
                    ...
                    var select = viewPortSelect;

                    if (playerCount < 4 && i == 2) { select -= 2; }

                    for(int j = 0; j < cam.Length; ++j)
                    {
                        //setting the view port in teh camera component collect in the getComponent line
                        cam[j].rect = new Rect(viewPort[i, select],
                                               viewPort[i, select + 1], 1f, 1f);
                    }
